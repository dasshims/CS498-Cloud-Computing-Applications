...
public class PopularityLeague extends Configured implements Tool {
    ...
    public static class LinkCountMap extends Mapper<Object, Text, IntWritable, IntWritable> {
        ...
        protected void setup(Context context) throws IOException,InterruptedException {
            ...
            List<String> stopWordsTmp = Arrays.asList(readHDFSFile(league, conf).split("\n"));
    ...
    public static class TopLinksReduce extends Reducer<NullWritable, IntArrayWritable, IntWritable, IntWritable> {
        ...
        public void reduce(NullWritable key, Iterable<IntArrayWritable> values, Context context) throws IOException, InterruptedException {
            ...
            /*for (IntArrayWritable val : values) {
                ...
                IntWritable[] pair = new IntWritable[writables.length];
                for (int i = 0; i < writables.length; i++) {
                    pair[i] = (IntWritable) writables[i];
                }
            ...
            TreeSet<Pair<Integer, Integer>> countTopLeagueMap = new TreeSet<Pair<Integer, Integer>>();
            for (IntArrayWritable val : values) {
                Writable[] writables = val.get();
                IntWritable[] pair = new IntWritable[writables.length];
                for (int i = 0; i < writables.length; i++) {
                ...
                //map.put(count, head);
                countTopLeagueMap.add(new Pair<Integer, Integer>(head, count));
            }

            /*int rank = 0;
            for (Integer head : map.descendingMap().values()) {
                context.write(new IntWritable(head), new IntWritable(rank++));
            }*/
